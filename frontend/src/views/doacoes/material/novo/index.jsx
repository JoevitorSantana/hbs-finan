...
const NovaDoacaoMaterial = () => {
    ...
    const fetchDados = async () => {
      ...
      try {
        const [resProdutos, resFuncionarios] = await Promise.all([
          fetch("http://localhost:8080/produtos/listar-sem-data", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Doação de material cadastrada com sucesso!");
  ...
  return (
    <Row>
      <Col sm={12}>
        <Card>
          <Card.Header>
            <Card.Title as="h5">Nova Doação de Material</Card.Title>
          </Card.Header>
          <Card.Body>
            ...
            {loading ? (
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Button variant="outline-primary" onClick={adicionarProduto} className="mb-3">
                  + Adicionar Produto
                </Button>

                <Row className="mt-3">
                  <Col>
                    <Button variant="secondary" onClick={() => navigate("/doacao")} className="me-2">
                      Cancelar
                    </Button>
...
export default NovaDoacaoMaterial;
