...
const Doacoes = () => {
    ...
    const handleDeleteDoacao = async () => {
        try {
            const response = await fetch(`http://localhost:8080/doacao-monetaria/excluir/${doacaoSelecionada.id}`, {
                method: 'DELETE',
                headers: {
  ...
  return (
    ...
    doacao.id_ap.toString().includes(termo)  // busca pelo ID do apoiador
  );
});
                    ...
                    <Card>
                        <Card.Header>
                            <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Card.Title as="h5">Doações Monetárias</Card.Title>
                                <Link to="/doacao/monetaria/novo">
                                ...
                                <tbody>
                                    {doacoesFiltradas && doacoesFiltradas.map((doacao) => (
                                        <tr key={doacao.id}>
                                            ...
                                            <td>{doacao.data}</td>
                                            <td>{doacao.id_ap}</td>
                                            <td>{doacao.id_caixa}</td>
                                            <td>
                                                <Link to={`/doacao/monetaria/editar/${doacao.id}`}>
                                ...
                                <tbody>
                                    {doacoesFiltradas && doacoesFiltradas.map((doacao) => (
                                        <tr key={doacao.id}>
                                            ...
                                            <td>{doacao.forma_pagamento?.nome || doacao.forma_pagamento}</td>
                                            <td>
                                                <Link to={`/doacao/editar/${doacao.id}`}>
                                                    <Button size="sm" className="label theme-bg text-white f-12 me-1">
                                                        <FaEdit />
                                                    </Button>
                                                </Link>
...
const DeleteConfirmationModal = ({ show, onHide, onConfirm, doacao }) => (
    <Modal show={show} onHide={onHide} centered>
        ...
        <Modal.Body>
            Tem certeza que deseja excluir a doação de <strong>{doacao?.doador}</strong> no valor de <strong>R$ {doacao?.valor?.toFixed(2)}</strong>?
        </Modal.Body>
        <Modal.Footer>
